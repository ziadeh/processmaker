    ...
    <b-card-header>
        <b-row>
                ...
                <template v-for="(item, index) in section.left">
                    <b-button-group v-if="isVisible(item, 'group')" size="sm" :key="index">
                        <b-button v-for="(button, indexButton) in item.items" :variant="button.variant || 'secondary'"
                                  ...
                                  @click="button.action" v-if="button.hide !== true">
                            <i :class="button.icon"></i> {{ button.name }}
                        </b-button>
                    </b-button-group>

                    <b-button v-if="isVisible(item, 'button')" :variant="item.variant || 'secondary'"
                              size="sm"
                              class="text-capitalize" :title="item.title" :key="index" @click="item.action">
                        <i :class="item.icon"></i> {{ item.name }}
                    </b-button>

                    <component v-if="item.type !== 'group' && item.type !== 'button'" :is="item.type" :options="item.options" :key="index"></component>
            ...
            <b-col class="text-right">
                    ...
                    <b-button v-if="isVisible(item, 'button')" size="sm"
    ...
    computed: {
      section () {
        ...
        this.options.forEach(item => {
          ...
          response[item.section].push(Object.assign(item, this.changeItems[item.id]));
