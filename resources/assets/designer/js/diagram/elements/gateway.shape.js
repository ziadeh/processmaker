    ...
    getDecorator (type, options) {
        ...
        switch (type) {
            ...
            case "bpmn:ExclusiveGateway":
                decorator = this.canvas.group(
                    this.canvas.polyline([+options.x - 8, +options.y - 8, +options.x + 8, +options.y + 8]),
                    this.canvas.polyline([+options.x - 8, +options.y + 8, +options.x + 8, +options.y - 8])
                ).attr(this.attr);
                break;
            case "bpmn:ParallelGateway":
                decorator = this.canvas.group(
                    this.canvas.polyline([+options.x - 12, +options.y, +options.x + 12, +options.y]),
                    this.canvas.polyline([+options.x, +options.y - 12, +options.x, +options.y + 12])
                ).attr(this.attr);
                break;
            case "TEXT":
