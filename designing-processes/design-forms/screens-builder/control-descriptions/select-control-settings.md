...
* **Read Only:** Select the **Read Only** checkbox to indicate that the Select control cannot be edited. This option is not selected by default.
...
* **Label:** Enter the text label that displays for this control. **New Select** is the default value.
...
From the **Data Source** panel, select one of the following methods to specify options that display in the Select control:
